  ...
  def buddy_icon(user,size = nil )
    ...
    # check user_buddy_type
      if user.buddy_icon  # user uploaded its icon
        ...
        src = gravatar_url(user.email, grav_size, user.gender)
      end
    return image_tag_without_asset_host(src,:class => "photo" )
  end
  
  def gravatar_url(email,grav_size, gender = nil)
      woman_case = ( gender == 2 ) ? "-w" : ""
